const First = require("../model/first_resp.model");
...
class Controller {
  async get(req, res) {
    ...
    console.log("first");
    try {
      const model = await First.find({});
      const { name } = await verify(req.cookies.id);
      log("First: получение first", name, Ip.address(), "GET");
      return res.json(model);
    } catch (e) {
      //при ошибке занос в логи
      const { name } = await verify(req.cookies.id, config.get("jwt"));
      log("First: " + e.toString(), name || "error", Ip.address(), "GET");
      return res.send(e);
    }
  }
  async getId(req, res) {
    ...
    try {
      ...
      if (str === ".xml") {
        ...
        log("First: получение first по id xml", name || error, Ip.address(), "GET");
        return res.send(util(jsonData));
      }
      log("First: получение first по id json", name || error, Ip.address(), "GET");
  ...
  async getIds(req, res) {
    ...
    try {
      ...
      for (let i = 0; i <= ids.length - 1; i++) {
        if (ids[i] === "") continue;
        const model = await First.findById(ids[i]);
        arr.push(model);
      }
      const { name } = await verify(req.cookies.id, config.get("jwt"));
      log("First: получение по id's", name || error, Ip.address(), "GET");
      return res.json(arr);
    } catch (e) {
      //при ошибке занос в логи
      log("First: " + e.toString(), null, Ip.address(), "GET");
      return res.send(e);
    }
  }
  async add(req, res) {
    ...
    try {
      ...
      try {
        const { name } = await verify(req.cookies.id, config.get("jwt"));
        log("First: создание first", name || "error", Ip.address(), "GET");
      } catch (e) {
        console.log(e);
      }
      ...
      //при ошибке занос в логи
      log("First: " + e.toString(), null, Ip.address(), "POST");
      return res.send(e);
    }
  }
  async save(model) {
    const first = new First(model);
    await first.save();
    pg.first(model);
    return first._id;
  }
