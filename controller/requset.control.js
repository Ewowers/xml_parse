...
class Controller {
  async get(req, res) {
    //get all type json
    try {
      const model = await Request.find({});
      const { name } = await verify(req.cookies.id);
      log("Request: получение request", name, Ip.address(), "GET");
  ...
  async getId(req, res) {
    ...
    try {
      ...
      if (str === ".xml") {
        ...
        log("Request: получение request по id xml", name || error, Ip.address(), "GET");
        return res.send(util(jsonData));
      }
      log("Request: получение request по id json", name || error, Ip.address(), "GET");
  ...
  async getIds(req, res) {
    ...
    try {
      ...
      for (let i = 0; i <= ids.length - 1; i++) {
        if (ids[i] === "") continue;
        const model = await Request.findById(ids[i]);
        arr.push(model);
      }
      const { name } = await verify(req.cookies.id, config.get("jwt"));
      log("Request: получение по id's", name || error, Ip.address(), "GET");
      return res.json(arr);
    } catch (e) {
      //при ошибке занос в логи
      log("Request: " + e.toString(), null, Ip.address(), "GET");
      return res.send(e);
    }
  }
  async add(req, res) {
    ...
    try {
      ...
      try {
        const { name } = await verify(req.cookies.id, config.get("jwt"));
        log("Request: создание request", name || "error", Ip.address(), "GET");
      } catch (e) {
        console.log(e);
      }
      ...
      //при ошибке занос в логи
      log("Request: " + e.toString(), null, Ip.address(), "POST");
      return res.send(e);
    }
  }
  async save(model) {
    const request = new Request(model);
    await request.save();
    pg.request(model);
    return request._id;
  }
